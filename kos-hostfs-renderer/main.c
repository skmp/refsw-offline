...
int main(int argc, char **argv) {
    ...
    printf("pvr reg load ...\n");
    {
        FILE* regs = fopen("/pc/pvr_regs.bin", "rb");
        if (!regs) {
            printf("failed to open /pc/pvr_regs.bin\n");
            return -1;
        }
    ...
    printf("pvr mem load ...\n");
    {
        FILE* mem = fopen("/pc/pvr_mem.bin", "rb");
        if (!mem) {
            printf("failed to open /pc/pvr_mem.bin\n");
            return -1;
        }
    ...
    */
    ...
    printf("pvr reg store ...\n");
    {
        FILE* regs = fopen("/pc/pvr_regs.out", "wb");
        if (!regs) {
            printf("failed to open /pc/pvr_regs.out\n");
            return -1;
        }
        ...
        for (int i = 0; i < 2048; i++) {
            ...
            reg_val = PVR_REG(reg_num);
            fwrite(&reg_val, sizeof(reg_val), 1, regs);
        }

        fclose(regs);
    }

    printf("pvr mem store ...\n");
    {
        FILE* mem = fopen("/pc/pvr_mem.out", "wb");
        setvbuf(mem, NULL, _IONBF, 0); // See https://dcemulation.org/phpBB/viewtopic.php?p=1060227#p1060227
        if (!mem) {
            printf("failed to open /pc/pvr_mem.out\n");
            return -1;
        }
        fwrite((void*)PVR_MEM_32_BASE, PVR_MEM_SIZE, 1, mem);
        fclose(mem);
    }
