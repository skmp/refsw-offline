...
#define FLUSH_NAN(a) isnan(a) ? 0 : a
        ...
        for (int y = spany; y > 0; y -= 1)
        {
            ...
            for (int x = spanx; x > 0; x -= 1)
            {
                ...
                if (inTriangle)
                {
                    ...
                    PixelFlush_isp(this, render_mode, params->isp.DepthMode, x_ps, y_ps, invW, cb_x, tag);
                }
    ...
    static void PixelFlush_isp(refsw* backend, RenderMode render_mode, u32 depth_mode, float x, float y, float invW, u8 *pb, parameter_tag_t tag)
    {
...
Renderer* rend_refsw(u8* vram) {
    return rend_refred_base(vram, [=]() { 
        return (RefRendInterface*) new refsw_impl(vram);
    });
}
